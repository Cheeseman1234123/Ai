...
def get_basic_caption(image_path: str) -> str:
    ...
    cap, err = _run_models(VISION_MODELS, msgs, max_tokens=150, temperature=0.2)
...
""")
def main():
    image_path = input(f"{Fore.BLUE}Enter the path of the image (e.g., test.jpg): {Style.RESET_ALL}").strip()
    ...
    while True:
        ...
        if choice == "1":
            ...
            prompt = (f"Rewrite the following description into EXACTLY 30 words. "
                      f"It must be a single sentence. Text: {basic_caption}")
            print(f"{Fore.CYAN}Refining to 30 words...")
            out = generate_exact_sentence(prompt, 30, max_new_tokens=150)
            print(f"{Fore.GREEN}✅ Description (30 words): {Fore.YELLOW}{Style.BRIGHT}{out}\n")
        elif choice == "3":
            prompt = (f"Rewrite the following description into EXACTLY 50 words. "
                      f"It must be a single sentence. Text: {basic_caption}")
            print(f"{Fore.CYAN}Refining to 50 words...")
            out = generate_exact_sentence(prompt, 50, max_new_tokens=200)
