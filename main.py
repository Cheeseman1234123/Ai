from groq import generate_sequence
def run_activity():
    print("ZERO-SHOT, ONE-SHOT & FEW-SHOT LEARNING ACTIVITY")
    category = input("Enter a category (e.g., animals, food, city): ").strip()
    ...
    print("\n---ZERO-SHOT LEARNING---")
    print(f"Response: {generate_sequence(zero_shot, temperature=0.3, max_tokens=1024)}")
    one_shot = f"""Example:
Category: fruit
Item: apple
...
Answer:"""
    print("\n---ONE-SHOT LEARNING---")
    print(f"Response: {generate_sequence(one_shot, temperature=0.3, max_tokens=1024)}")
...
Answer:"""
    print("\n---FEW-SHOT LEARNING---")
    print(f"Response: {generate_sequence(few_shot, temperature=0.3, max_tokens=1024)}")
...
Story:"""
    ...
    print(f"Response{generate_sequence(few_shot, temperature=0.7, max_tokens=1024)}")
    print("\n---REFLECTION QUESTIONS---")
    print("1. How did the response differ between zero-shot, one-shot and few-shot?")
    print("2. Which approach gave the most helpful answers?")
