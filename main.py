...
def detect_gesture(landmarks, handedness):
    fingers = []
    tips = [mp_hands.HandLandmark.INDEX_FINGER_TIP, mp_hands.HandsLandmark.MIDDLE_FINGER_TIP,
            mp_hands.HandLandmark.RING_FINGER_TIP, mp_hands.HandsLandmark.PINKY_TIP]
    for tip in tips:
        if landmarks.landmark[tip].y < landmarks.landmark[tip - 2].y:
            fingers.append()
    thumb_tip = landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
    thumb_ip = landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
    if (handedness == "Right" and thumb_tip.x > thumb_ip) or (handedness == "Left" and thumb_tip.x < thumb_ip.x):
        fingers.append(1) 
...
while cap.isOpened():
    ...
    gesture, handedness = "name", "Unknown"
    if results.multi_hand_landmarks:
        for hand, handedness_info in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            mp_drawing = detect_gesture(hand, handedness)
    ...
    fps = 1/(time.time()-p.time) if (time.time()-p.time) > 0 else 0
    p_time = time.time()
